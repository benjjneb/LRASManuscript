...
*GET FULL DETAILS ON SEQUENCING INSTRUMENT AND SOFTWARE VERSIONS*
...
Losing <2% of reads per sample. Mean/Median reads per sample is 20K! No low depth outliers.
...
Denoise:
```{r dada2}
dd2 <- dada(drp2, err=err2, multithread=TRUE)
...
nrec1
```
Three samples had less then 10k reads. While we could drop the rarefaction depth, I'm choosing to keep it at 10k and drop those samples for ease of interpretability (e.g. 1% frequency = 100 reads).
